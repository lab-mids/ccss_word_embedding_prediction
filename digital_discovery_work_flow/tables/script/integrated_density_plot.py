...
class IntegratedDensityCombinedXPlot:
    def __init__(self, filepaths, columns, plot_types, legend_labels=None, fontsize=14,
                 subplot_width=7, subplot_height=5):
        """
        ...
        :param plot_types: List of plot types ('xy' or 'x_only') corresponding to each file.
        ...
        self.legend_labels = legend_labels if legend_labels else [col for i, col in
                                                                  enumerate(columns) if
                                                                  plot_types[
                                                                      i] == 'x_only']
    ...
    def plot_all(self):
        ...
        subplot_labels = [f"({chr(97 + i)})" for i in
                          range(2)]  # Only two plots (xy and combined x)
        plot_index = 0

        combined_ax = axes[1]  # Combined x plot at the second position
        secondary_y = None
        color_map = plt.get_cmap('tab10',
                                 len(self.legend_labels))  # Use different colors for combined x plots

        x_only_index = 0  # To keep track of legend labels for x_only plots
        secondary_legends = []  # To collect legend entries for secondary y-axis (Similarity)
        ...
        for i, (df, column, plot_type) in enumerate(
            ...
            # If this is the first xy plot, calculate the direction based on max and min points
            if i == 0 and plot_type == 'xy':
                ...
                self.direction = direction / np.linalg.norm(
                    direction)  # Store the normalized direction vector
            ...
            start_point = min_point - norm_direction * (
                        np.abs(min_point[0] - x_range[0]) + np.abs(
                    min_point[1] - y_range[0]))
            end_point = max_point + norm_direction * (
                        np.abs(max_point[0] - x_range[1]) + np.abs(
            ...
            if plot_type == 'xy':
                ...
                ax.scatter(x, y, c='gray',
                           alpha=0.3)  # Plot original data points in gray
                ...
                cbar.set_label(f'Interpolated {column} (mA/cm²)',
                               fontsize=self.fontsize)
                cbar.ax.tick_params(labelsize=self.fontsize - 2)

                # Add subplot labels outside the plot area
                ax.text(-0.15, 1.05, subplot_labels[plot_index], transform=ax.transAxes,
                        fontsize=self.fontsize + 2, fontweight='bold', va='center',
                        ha='right')

                plot_index += 1

            elif plot_type == 'x_only':
                # Combine all x_only plots in the same subplot with different line colors
                ...
                if column == 'Similarity':
                    ...
                    similarity_line, = secondary_y.plot(line_x, line_current,
                                                        label=label,
                                                        color=color_map(x_only_index),
                                                        lw=2)
                    secondary_y.tick_params(axis='both', which='major',
                                            labelsize=self.fontsize - 2)
                    secondary_legends.append(similarity_line)
                else:
                    combined_ax.plot(line_x, line_current, label=label,
                                     color=color_map(x_only_index), lw=2)
        ...
        combined_ax.legend(handles + secondary_legends,
                           labels + [legend.get_label() for legend in
                                     secondary_legends], loc='lower right',
                           fontsize=self.fontsize - 2)

        combined_ax.tick_params(axis='both', which='major', labelsize=self.fontsize - 2)

        # Add label to the combined x plot
        combined_ax.text(-0.1, 1.05, subplot_labels[plot_index],
...
def parse_args():
    ...
    parser.add_argument("--plot_types", nargs="+", required=True,
                        help="List of plot types for each file (xy or x_only)")
    parser.add_argument("--legend_labels", nargs="+",
    ...
    parser.add_argument("--subplot_width", type=int, default=7,
                        help="Width of each subplot in inches (default: 7)")
    parser.add_argument("--subplot_height", type=int, default=5,
