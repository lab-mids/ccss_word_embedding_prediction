    ...
    def analyze(self):
        ...
        for i, config in enumerate(self.df_configs):
            ...
            possible_metrics = {
                ...
                "Standard Deviation (Predicted)": lambda actual, predicted: predicted.std(),  # noqa
                ...
                "Overall coefficient of determination (R^2)": lambda actual, predicted: 1  # noqa
                - (
                    np.sum((actual - predicted) ** 2)
                    / np.sum((actual - np.mean(actual)) ** 2)
                ),
                "Overall Correlation (r)": lambda actual, predicted: np.corrcoef(
                    actual, predicted
                )[0, 1],
            }
            ...
            if filter_col and filter_value is not None:
                ...
                if not filtered_actual.empty:
                    possible_metrics[
                        f"Correlation (r) for {filter_col} < {filter_value}"
                    ] = lambda actual, predicted: np.corrcoef(
                        filtered_actual, filtered_predicted
                    )[
                        0, 1
                    ]
        ...
        results_df.columns = [
            config.get("name", f"DataFrame {i + 1}")
            for i, config in enumerate(self.df_configs)
        ]
        return results_df
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze predictions and compute metrics."
    )
    parser.add_argument(
        "--data_paths", nargs="+", required=True, help="Paths to the CSV files."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Name of the column for actual values.",
    )
    parser.add_argument(
        "--predicted_col",
        type=str,
        required=True,
        help="Name of the column for predicted values.",
    )
    parser.add_argument(
        "--filter_col", type=str, help="Column to apply the filter (optional)."
    )
    parser.add_argument(
        "--filter_value", type=float, help="Value for filtering data (optional)."
    )
    parser.add_argument(
        "--metrics_to_skip", nargs="+", help="List of metrics to skip (optional)."
    )
    parser.add_argument(
        "--output_file",
        type=str,
