...
class ElementConcentrationPlotter:
    ...
    def plot(self):
        ...
        for data_index, data in enumerate(reversed(self.dataframes)):
            elements_in_data = [col for col in data.columns if
                                col in self.periodic_table]
            sorted_elements = self.get_sorted_elements(elements_in_data)
            positions = self.adjust_positions(current_position, len(sorted_elements),
                                              element_offset)
            ...
            for i, element in enumerate(sorted_elements):
                ...
                plt.plot(min_val, position, marker_shape, markersize=10,
                         color=line_color, clip_on=False)
                plt.plot(max_val, position, marker_shape, markersize=10,
                         color=line_color, clip_on=False)

                # Adjust text to prevent crossing axis
                offset = 1.5  # Adjust this value as needed to avoid crossing the axes
                plt.text(min_val - offset, position, f'{min_val}',
                         verticalalignment='center',
                         horizontalalignment='right', color=line_color, fontsize=15)
                plt.text(max_val + offset, position, f'{max_val}',
                         verticalalignment='center',
                         horizontalalignment='left', color=line_color, fontsize=15)

            plt.text(-10, sum(positions) / len(positions), system_label,
                     verticalalignment='center', horizontalalignment='right',
                     fontsize=15, color='black', weight='bold')

            if data_index < len(self.dataframes) - 1:
                current_position += len(
                    sorted_elements) * element_offset + system_offset
        ...
        custom_lines = [
            mlines.Line2D([0], [0], color=color, marker=self.element_marker_map[elem],
                          markersize=8, linestyle='-', lw=3) for elem, color in
            self.element_color_map.items()]
        plt.legend(custom_lines, self.element_color_map.keys(), loc='lower right',
                   fontsize='large')  # Adjusted legend text size
...
def parse_args():
    ...
    parser.add_argument("--data1", type=str, required=True,
                        help="Path to the first dataset CSV file")
    parser.add_argument("--data2", type=str, required=True,
                        help="Path to the second dataset CSV file")
    parser.add_argument("--data3", type=str, required=True,
                        help="Path to the third dataset CSV file")
    parser.add_argument("--output", type=str, required=True,
                        help="Output path for the plot PDF file")
