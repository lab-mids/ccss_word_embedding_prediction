    ...
    def standardize_columns(self, data, element_columns):
        for col in element_columns:
            if col not in data.columns:
                data[col] = 0
        return data[element_columns + [col for col in data.columns if col not in element_columns]]
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a GPy model for current density predictions.")
