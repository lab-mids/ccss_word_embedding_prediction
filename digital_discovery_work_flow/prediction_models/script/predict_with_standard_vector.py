...
class MaterialVectorAnalysis:
    ...
    def filter_rows_by_training_data(df, training_data_values):
        training_data_str_values = [str(value) if isinstance(value, list) else value for
                                    value in training_data_values]
    ...
    def load_and_prepare_data(self):
        ...
        self.datasets[2] = self.datasets[2][
            ...
            (abs(self.datasets[2]['Current_at_850mV'] <= 1))].copy()

        self.results_df = pd.read_csv(self.results_path)
        self.results_df = self.convert_vector_column_to_ndarray(self.results_df,
                                                                'Standard Vector')

    def calculate_similarity(self, target_data_index, elements,
                             target_vec_column='Standard Vector',
                             training_data_values=[['data1', 'data2']]):
        filtered_data = self.filter_rows_by_training_data(self.results_df,
        ...
        return self.calculator.calculate_similarity_from_dataframe(
            self.datasets[target_data_index], elements, target_vec=target_vec,
    ...
    def plot_similarity(self, df, elements, cmap_groups):
        return self.visualizer.plot_similarity_scatter(df, elements=elements,
...
def parse_args():
    ...
    parser.add_argument("--data_paths", nargs='+', required=True,
                        help="Paths to the dataset CSV files.")
...
def main():
    ...
    df_similarity = analysis.calculate_similarity(target_data_index=2,
