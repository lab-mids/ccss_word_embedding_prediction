...
class MaterialAnalysis:
    ...
    def compute_material_vector(self, row):
        ...
        multiplied_vectors = [
            self.model.wv[element.lower()] * count for element, count in elements_counts
        ]
    ...
    def preprocess_data(self):
        ...
        for i, dataset in enumerate(self.datasets):
            dataset["Material_Vector"] = dataset.apply(
                lambda row: self.compute_material_vector(row), axis=1
            )
            self.datasets[i] = dataset

    def similarity_difference(
        self, weights, similarities, property_vectors, standard_vec
    ):
        weighted_standard_vec = np.dot(weights, property_vectors)
        weighted_standard_vec = weighted_standard_vec.reshape(1, -1)
        similarities_to_weighted_standard = cosine_similarity(
            standard_vec, weighted_standard_vec
        ).flatten()
    ...
    def optimize_weights(self):
        ...
        property_vectors = [
            np.mean([self.model.wv[word] for word in prop.split()], axis=0)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Optimize weights of properties and obtain a standard vector."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="List of properties for optimization.",
    )
    parser.add_argument(
        "--output_file",
        type=str,
        required=True,
        help="Output CSV file to save optimization results.",
    )
...
def main():
    ...
    # Initialize the MaterialAnalysis with model path, data paths, and property list
